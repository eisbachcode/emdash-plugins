...
/**
 * The widget exists for editors, so the route has to say so: an `admin`
...
describe("saving settings", () => {
	...
	it("goes through for an administrator", async () => {
		...
		expect(scheduled).toContain("sweep");
		expect(result.toast).toBeUndefined();
	});

	it("stores nothing when emdash refuses the schedule", async () => {
		...
		const result = (await adminHandler()(
			{
				input: { type: "form_submit", action_id: "save_settings", values: { schedule: "every night" } },
				request: { url: "https://example.test", method: "POST", headers: {} },
				user: { id: "u1", email: "u@example.test", name: null, role: ROLE.ADMIN },
			} as never,
			refusing,
		)) as { toast?: { message: string; type: string } };
		expect(writes).toHaveLength(0);
